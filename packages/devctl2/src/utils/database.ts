 ...
 */
async function getDatabaseUrlFromEnv(config: DevCtl2Config, workdir: string): Promise<string | null> {
  // Try to find DATABASE_URL in any .env file
  const envFilePaths = [
    config.apps.api?.envFile ? path.join(workdir, config.apps.api.envFile) : null,
