...
import type { Ports, RouteInfo } from '../types.js';
  ...
  async listRoutes(baseDomain: string): Promise<RouteInfo[]> {
    ...
    for (const route of routes) {
      if (route.match?.[0]?.host?.[0]?.includes(baseDomain)) {
        ...
        // Extract port info from route headers
        ...
        const apiPort = apiRoute?.handle[0].upstreams[0].dial.split(':')[1] || 'N/A';
        const webPort = webRoute?.handle[0].upstreams[0].dial.split(':')[1] || 'N/A';
        const workPath = apiRoute?.handle[0].headers?.response?.set?.['X-Worktree-Path']?.[0] || 'N/A';
