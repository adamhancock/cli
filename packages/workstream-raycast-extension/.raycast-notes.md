...
2. Test in Raycast:
   - Initial load is **instant** (< 10ms)
   - **Real-time updates** via WebSocket connection
   - Search bar shows "⚡ Real-time" indicator when connected
   - Status updates appear **automatically** without manual refresh
   - Force refresh with **Cmd+R** (triggers daemon to re-poll immediately)
...
Workstream Daemon (background service, polls every 5s)
  ↓
  ├─ WebSocket (ws://localhost:58234) ───→ Raycast Extension (real-time updates)
  └─ Cache (~/.workstream-daemon/instances.json) ───→ Fallback if WS fails
```

**Real-time mode**: Raycast maintains a persistent WebSocket connection and receives instant updates whenever the daemon detects changes (git status, PR updates, Claude state changes, etc.). No manual refresh needed!
...
### Slow loading or no real-time updates
...
- Look for "⚡ Real-time" indicator in search bar
- Check Raycast extension logs for WebSocket connection errors
- If WebSocket fails, extension automatically falls back to cache file mode
